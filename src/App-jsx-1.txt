...
import { LanguageProvider, useLanguage, COUNTRIES } from "./lib/i18n";
...
function PhoneLoginScreen({ onLoggedIn }) {
  ...
  const [countryIdx, setCountryIdx] = useState(0); // ডিফল্ট: Bangladesh
    ...
    <div className="w-full h-full flex flex-col items-center justify-center px-6" style={{ background: CREAM }}>
      ...
      <div className="w-full max-w-[340px]">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-bold text-[20px]">{tt("appName")}</h1>
          <p className="text-gray-500 text-[13px] mt-1">
            {step === "phone" ? tt("loginPhonePrompt") : tt("loginOtpPrompt")}
          </p>
        </div>
        ...
        {step === "phone" ? (
          <>
            <div className="mb-3">
              <label className="text-[11px] text-gray-400 mb-1 block">{tt("selectCountry")}</label>
              <select
                value={countryIdx}
                onChange={(e) => setCountryIdx(Number(e.target.value))}
                className="w-full border rounded-xl px-3 py-3 bg-white text-[14px] outline-none"
              >
                {COUNTRIES.map((c, i) => (
            ...
            <label className="text-[11px] text-gray-400 mb-1 block">{tt("phoneNumber")}</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-3 mb-3 bg-white">
              <Phone size={16} className="text-gray-400" />
              <span className="text-[14px] text-gray-500">+{country.dial}</span>
              <input
                value={phone}
            ...
            >
              {sending ? tt("sending") : tt("sendCode")}
            </button>
          </>
        ) : (
          <>
            ...
            <button
              ...
              style={{ background: ORANGE }}
            >
              {sending ? tt("verifying") : tt("verify")}
            </button>
            <button
