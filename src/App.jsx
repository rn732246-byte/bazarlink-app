...
function PhoneLoginScreen({ onLoggedIn }) {
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("কোড পাঠাতে সমস্যা হয়েছে — নম্বর ঠিক আছে কিনা দেখুন, আবার চেষ্টা করুন");
    } finally {
      setSending(false);
    }
  };
  ...
  const verifyCode = async () => {
    ...
    try {
      ...
      console.error(err);
      setError("কোড মিলছে না — আবার চেষ্টা করুন");
    } finally {
      setSending(false);
    }
  };
    ...
    <div className="w-full h-full flex flex-col items-center justify-center px-6" style={{ background: CREAM }}>
      ...
      <div className="w-full max-w-[340px]">
        <div className="text-center mb-8">
          <div className="w-14 h-14 rounded-2xl mx-auto mb-3 flex items-center justify-center" style={{ background: ORANGE }}>
            <Store size={26} color="white" />
          </div>
          <h1 className="font-bold text-[20px]">বাজারলিংক</h1>
          <p className="text-gray-500 text-[13px] mt-1">
            {step === "phone" ? "শুরু করতে আপনার ফোন নম্বর দিন" : "নম্বরে পাঠানো ৬ সংখ্যার কোড দিন"}
          </p>
        </div>
        ...
        {step === "phone" ? (
          <>
            ...
            >
              {sending ? "পাঠানো হচ্ছে..." : "কোড পাঠান"}
            </button>
          </>
        ) : (
          <>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-3 mb-3 bg-white">
              <input
                ...
                inputMode="numeric"
                maxLength={6}
                className="flex-1 outline-none text-[14px] tracking-widest"
              />
            </div>
            {error && <p className="text-red-500 text-[13px] mb-3">{error}</p>}
            <button
              ...
              style={{ background: ORANGE }}
            >
              {sending ? "যাচাই হচ্ছে..." : "যাচাই করুন"}
            </button>
            <button
              onClick={() => { setStep("phone"); setCode(""); setError(""); }}
              className="w-full text-gray-500 text-[13px] py-3"
            >
              নম্বর পরিবর্তন করুন
            </button>
          </>
  ...
  return (
    <div style={{ background: ORANGE }} className="px-4 py-3 flex items-center justify-between">
      <div className="flex items-center gap-2 text-white">
        ...
        <Gift size={20} />
        <span className="font-semibold text-[17px]">বাজারলিংক</span>
      </div>
...
function BottomNav({ active, onChange }) {
  const items = [
    { key: "dashboard", label: "ড্যাশবোর্ড", icon: BarChart3 },
    { key: "products", label: "পণ্য", icon: Package },
    { key: "orders", label: "অর্ডার", icon: Receipt },
    { key: "ledger", label: "খাতা", icon: BookOpen },
    ...
    <div className="min-h-full flex flex-col bg-white px-6 pt-8 pb-6">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <div className="text-[26px] font-bold leading-none">
            Shop<span className="text-pink-600">Easy</span>
          </div>
          <div className="text-gray-400 text-[12px]">স্মার্ট শপিং, সহজ জীবন</div>
        </div>
      </div>
      <h1 className="text-[22px] font-bold mt-6 mb-1">স্বাগতম!</h1>
      <p className="text-gray-500 text-[14px] mb-6 leading-relaxed">
        আপনার প্রয়োজনীয় পণ্য এখন আরও সহজে
      </p>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="font-bold text-[17px]">পাইকারি</div>
            <div className="text-gray-500 text-[12px] leading-snug">
              আমি পাইকারি ব্যবসায়ী<br />(বড় পরিমানে পণ্য কিনতে ও বিক্রি করতে চাই)
            </div>
          </div>
        </div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="font-bold text-[17px]">দোকানদার</div>
            <div className="text-gray-500 text-[12px] leading-snug">
              আমি দোকানদার<br />(খুচরা পণ্য কিনে আমার দোকানে বিক্রি করতে চাই)
            </div>
          </div>
        </div>
...
function ProfileTab({ form, verified, role, onUpdate, onSignOut }) {
  ...
  return (
    <div className="px-4 pt-6 pb-6">
      ...
      <button onClick={onSignOut} className="w-full text-red-500 text-[13px] font-medium py-3 mt-4">
        লগআউট করুন
      </button>
...
function ShopProfileTab({ form, onUpdate, onSignOut }) {
  const [editing, setEditing] = useState(false);
...
function ShopBottomNav({ active, onChange, cartCount }) {
  const items = [
    { key: "home", label: "হোম", icon: HomeIcon },
    { key: "category", label: "ক্যাটাগরি", icon: Grid3x3 },
    { key: "cart", label: "কার্ট", icon: ShoppingCart, badge: cartCount },
    { key: "reels", label: "রিল", icon: PlayCircle },
    { key: "profile", label: "প্রোফাইল", icon: User },
  ];
  return (
    <div className="border-t bg-white flex justify-between px-1 py-2">
...
export default function BazarlinkPrototype() {
  const { uid, ready, signOut } = useAuthUser();
    ...
    body = (
      <div className="flex flex-col h-full">
        <div className="flex-1 overflow-y-auto" style={{ background: "#f7f8fa" }}>
          ...
          {shopTab === "profile" && <ShopProfileTab form={form} onUpdate={saveProfile} onSignOut={signOut} />}
        </div>
    ...
    body = (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto" style={{ background: CREAM }}>
          ...
          {tab === "profile" && <ProfileTab form={form} verified={verified} role={role} onUpdate={saveProfile} onSignOut={signOut} />}
        </div>
